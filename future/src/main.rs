...
impl IOSelector {
    fn new() -> Arc<Self> {
        let s = IOSelector {
            ...
            queue: Mutex::new(VecDeque::new()), // IOのキュー
    ...
    fn rm_event(&self, fd: RawFd, wakers: &mut HashMap<RawFd, Waker>) {
        let epoll_delete = EpollOp::EpollCtlDel;
        let mut ev = EpollEvent::new(EpollFlags::empty(), fd as u64);
        epoll_ctl(self.epfd, epoll_delete, fd, &mut ev).ok();
        wakers.remove(&fd);
    }
    ...
    fn select(&self) {
        ...
        while let Ok(nfds) = epoll_wait(self.epfd, &mut events, -1) {
            ...
            for n in 0..nfds {
                // eventfdからの通知だった場合、IOのキューからの通知であるためイベントをepollの監視に追加
...
impl<'a> Future for ReadLine<'a> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match self.reader.reader.read_line(&mut line) {
            ...
            Err(err) => {
                if err.kind() == std::io::ErrorKind::WouldBlock {
                    self.reader.selector.register(EpollFlags::EPOLLIN, self.reader.fd, cx.waker().clone());
                    Poll::Pending
                } else {
                    Poll::Ready(None)
                }
